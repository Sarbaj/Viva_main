...
setRelode("1")
   try {
    console.log(value);
    const UpdateViva = await fetch("http://localhost:5050/bin/update/vivadetail", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const HandleViewStudent=async(data,e)=>{
    ...
    try {
       const response = await fetch("http://localhost:5050/bin/get/studentinresult", {
          ...
          body: JSON.stringify({ classCode:teacherclasscodeid,student:data._id }),
        });
        const Data=await response.json()
        
        
        const responseViva = await fetch("http://localhost:5050/bin/get/all-viva", {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
          },
  ...
  return (
    <>
      <main className="MainDiv">
        ...
        <button onClick={(e)=>HandleDownloadExel(e)}>Download Exel</button>
          <div className="crossresult" onClick={()=>setstudentResultStatus(false)}>
            <p>X</p>
          </div>
           <table>
            <thead>
        ...
        <div className="uppr">
          ...
          <p>{studentData.length >0 ?("Joined Student : "+studentData.length):("No Student Joined Yet !")}</p>
        </div>
      ...
      <div className="student">
        ...
        <div className="lowers">
          <table>
            ...
            <tbody>
              {studentData.length >0 &&(studentData.map((data,i)=>{
              return <tr key={i}>
                <td>{data.name}</td>
                <td>{data.ennumber}</td>
