...
router.post("/get/student-class-count", async (req, res) => {
  ...
  try {
    const count = await classStudent.countDocuments({ student: studentId });
...
router.get("/admin/teachers", async (req, res) => {
  try {
    const teachers = await User.find({ role: "1" }).select("-password").sort({ createdAt: -1 });
    res.status(200).json({ success: true, teachers });
  } catch (error) {
    console.error("Error fetching teachers:", error);
    res.status(500).json({ message: "Server error" });
...
router.post("/admin/add-teacher", async (req, res) => {
  ...
  try {
    ...
    // Send email with credentials (NOT OTP - Welcome email)
    try {
      if (sendTeacherCredentialsEmail) {
        console.log("📧 Sending teacher credentials email to:", email);
        console.log("📧 Using function: sendTeacherCredentialsEmail (NOT sendOTPEmail)");
        
        const emailResult = await sendTeacherCredentialsEmail({
          name,
          email,
          ennumber,
          password // Plain text password for email
        });
        ...
        console.error("❌ sendTeacherCredentialsEmail function not available");
      }
    } catch (emailError) {
      console.error("❌ Error sending email:", emailError);
    }
