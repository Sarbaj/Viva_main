...
`;
  ...
  try {
    ...
    try {
      ...
      return res.status(400).json({ error: "Failed to parse JSON response from Gemini.", output });
    }
...
const Isstudent = await User.findById({_id:studentId });
  ...
  await newVivaResult.save();
   res.status(200).json({ message: "Successfully Result Created",data:newVivaResult });
...
router.post("/update/status", async (req, res) => {
  ...
  console.log(_id);
    ...
    const updatedUser = await resultModel.findByIdAndUpdate(_id,                      // only the ID here
  { active:false,score:marks,answers},       // fields to update
  { new: true }             // optional: returns updated document
);
    res.status(200).json({ message: "Successfully Updated" ,result:updatedUser});
});
...
router.post("/get/viva-info", async (req, res) => {
    ...
    const vivaRecords = await vivaModel.find({ classCode: { $in: classCodes } });
...
router.post("/get/viva-resultexist", async (req, res) => {
  ...
  try {
   ...
   return res.status(409).json({ message: "Viva ID not found" });
    ...
    res.status(200).json({ message: "Viva result ID exists.", result: Isvivaid });
  } catch (error) {
    return res.status(500).json({message:'Invalid Viva Try To Access'})
  }
...
router.post("/update/vivadetail", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: "Error Updating Viva", error: error.message });
  }
});
